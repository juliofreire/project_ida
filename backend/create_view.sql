...
taxa_com_lag AS (
    SELECT
        ...
        data,
        valor,
        LAG(valor) OVER (PARTITION BY servico, grupo_economico ORDER BY data) AS valor_mes_anterior
    FROM taxa_base
),
taxa_variacao AS (
    SELECT
        ...
        valor_mes_anterior,
        CASE
            WHEN valor_mes_anterior IS NULL OR valor_mes_anterior = 0 THEN NULL
            ELSE ROUND(((valor - valor_mes_anterior) / valor_mes_anterior)::numeric * 100, 2)
